 ...
 */
let userInputAge: String = "16e"

func checkAge(age: String) -> Int? {
    let intAge: Int? = Int(age)
    if let unwrappedAge = intAge {
        if unwrappedAge >= 18 {
            print("Welcome!")
        } else {
...
func costOfItem(_ nameOfItem: String) -> Double? {
    if let unwrappedStock = stock[nameOfItem] {
        if unwrappedStock > 0 {
